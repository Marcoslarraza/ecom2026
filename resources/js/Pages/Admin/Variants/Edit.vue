...
const form = useForm({
    ...
    options: props.variant.options.map(o => ({
        id: o.id,
        name: o.name,
        value: o.value,
...
const submit = () => {
    form.put(route('admin.variants.update', props.variant.id));
};
</script>

<template>
    <Head title="Editar Variante" />
    
    <AdminLayout>
        <template #header>Editar Variante: {{ variant.name }}</template>
                ...
                <div class="space-y-6">
                    ...
                    <div class="border-t pt-6">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-medium text-gray-900">Opciones y Precios</h3>
                            ...
                            <div v-for="(option, index) in form.options" :key="index" class="border rounded-lg p-4">
                                <div class="grid grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <input v-model="option.name" type="text" class="mt-1 block w-full rounded-lg border-gray-300" />
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Valor</label>
                                        <input v-model="option.value" type="text" class="mt-1 block w-full rounded-lg border-gray-300" />
                                    </div>
                                    <div>
